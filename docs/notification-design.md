...
找人。本次在**保留文本消息不变**的前提下，为每次推送追加红 / 黄 / 绿最多
三条**单篇** `news` 卡片消息（**按颜色分组，不是按患者**；#76 起每张卡片单独
一条消息——本机实测个人微信「企业会话」只渲染单篇 news，多篇会显示"暂不支持
此消息类型"，改为单篇后个人微信用户也能直接点进列表 / 详情；卡片封面固定取
`ALERT_LINK_BASE_URL/alert-cover.jpg`——`apps/web/public/alert-cover.jpg`，
1068×455 白底、院徽居中在中央 455×455 正方形内（企微显示宽横幅、个人微信
显示正方形中心裁切，两者都能保留完整院徽），dev 与生产构建路径一致，由
查看消息的客户端拉取；换封面只需替换该文件并重新构建；
