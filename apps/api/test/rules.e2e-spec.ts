...
describe('Rules API (e2e, real Postgres)', () => {
  ...
  it('DB availability probe (informational, always runs)', () => {
    if (!dbAvailable) {
      // eslint-disable-next-line no-console
      console.warn('rules.e2e-spec.ts: all subsequent cases are NO-OPS because no live Postgres was reachable.');
    }
    expect(true).toBe(true);
  });

  itWithDb('full lifecycle: create -> list -> get -> edit (in place) -> disable -> versioned edit', async () => {
    ...
    const listRes = await request(app.getHttpServer()).get('/api/rules').query({ keyword: '肿瘤' }).expect(200);
    expect(listRes.body.total).toBe(1);
    expect(listRes.body.items[0].id).toBe(ruleId);

    const getRes = await request(app.getHttpServer()).get(`/api/rules/${ruleId}`).expect(200);
    ...
    const versionedRes = await request(app.getHttpServer())
      .put(`/api/rules/${ruleId}`)
    ...
    const oldRow = await request(app.getHttpServer()).get(`/api/rules/${ruleId}`).expect(200);
    expect(oldRow.body.isEnabled).toBe(false);
  });

  itWithDb('rejects a duplicate enabled rule with RULE_CONFLICT and a conflictingRuleId', async () => {
  ...
  itWithDb('returns 409 RULE_VERSION_CONFLICT when retrying a semantic edit with an already-superseded version', async () => {
    ...
    const editorAResult = await request(app.getHttpServer())
  ...
  itWithDb('returns 409 RULE_VERSION_CONFLICT for a stale in-place edit too (e.g. two operators both disabling from a stale read)', async () => {
    const created = await request(app.getHttpServer())
      .post('/api/rules')
  ...
  itWithDb('returns 404 RULE_NOT_FOUND for an unknown id', async () => {
    const res = await request(app.getHttpServer()).get('/api/rules/00000000-0000-0000-0000-000000000000').expect(404);
  ...
  itWithDb('import: full success validates and confirms all rows', async () => {
    ...
    const listRes = await request(app.getHttpServer()).get('/api/rules').expect(200);
    expect(listRes.body.total).toBe(2);
  });

  itWithDb('import: partial failure reports per-row errors and only valid rows are importable', async () => {
  ...
  itWithDb('import: duplicate rows within the file are flagged, not silently deduped-and-imported', async () => {
    const csv = 'keyword,level,matchField\n癌,RED,REPORT_TEXT\n癌,RED,REPORT_TEXT\n';
