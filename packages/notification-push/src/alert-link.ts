 ...
 *
 * After the aggregate TEXT/NEWS template message, a run may send ONE extra
 * WeCom `news` message carrying up to three article cards - one per
 * RED / YELLOW / GREEN level that has at least one record in the summary
 * window. Each card links to the web app's `/alert` H5 page with an opaque
 * token; the api resolves the token to a frozen snapshot of that level's
 * monitor_record ids as of this run (membership never drifts afterwards).
 *
