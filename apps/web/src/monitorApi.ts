...
export async function listExams(query: ListMonitorExamsQuery): Promise<PaginatedMonitorExams> {
  ...
  return parseResponse(await fetch(`${API_BASE_URL}/api/monitor/exams?${params.toString()}`));
}
...
export async function getExamDetail(id: string): Promise<MonitorExamDetailDto> {
  return parseResponse(await fetch(`${API_BASE_URL}/api/monitor/exams/${encodeURIComponent(id)}`));
}

export async function getSyncStatus(): Promise<SyncStatusDto> {
  return parseResponse(await fetch(`${API_BASE_URL}/api/system/sync-status`));
}
