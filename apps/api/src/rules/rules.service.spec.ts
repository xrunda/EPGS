...
describe('RulesService', () => {
    ...
    const monitorRule = {
      ...
      findFirst: jest.fn(async ({ where, orderBy }: any) => {
        let candidates = Array.from(store.values()).filter((rule: any) => {
          ...
          if (where.keyword?.equals !== undefined && rule.keyword.toLowerCase() !== where.keyword.equals.toLowerCase()) return false;
          if (where.ruleGroupId?.not !== undefined && rule.ruleGroupId === where.ruleGroupId.not) return false;
          if (typeof where.ruleGroupId === 'string' && rule.ruleGroupId !== where.ruleGroupId) return false;
          return true;
        });
  ...
  describe('create', () => {
    it('creates a rule and sets ruleGroupId to its own id', async () => {
      const dto: any = { keyword: '肿瘤', level: 'RED', matchField: 'REPORT_TEXT', actorId: 'tester' };
    ...
    it('rejects a duplicate enabled rule with the same keyword/level/matchField/matchMode', async () => {
      makeRule({ keyword: '肿瘤', level: 'RED', matchField: 'REPORT_TEXT', matchMode: 'CONTAINS', isEnabled: true });
    ...
    it('keyword conflict check is case-insensitive (e.g. "Ca" vs "ca")', async () => {
      makeRule({ keyword: 'Ca', level: 'RED', matchField: 'REPORT_TEXT', matchMode: 'CONTAINS', isEnabled: true });

      const dto: any = { keyword: 'ca', level: 'RED', matchField: 'REPORT_TEXT', actorId: 'tester' };
      await expect(service.create(dto)).rejects.toBeInstanceOf(RuleConflictException);
    });

    it('does not conflict with a disabled rule of the same tuple', async () => {
      makeRule({ keyword: '肿瘤', level: 'RED', matchField: 'REPORT_TEXT', matchMode: 'CONTAINS', isEnabled: false });
    ...
    it('allows creating a disabled rule that would otherwise conflict', async () => {
      ...
      const dto: any = { keyword: '肿瘤', level: 'RED', matchField: 'REPORT_TEXT', actorId: 'tester', isEnabled: false };
      await expect(service.create(dto)).resolves.toBeDefined();
    });
  });
  ...
  describe('update', () => {
    it('updates in place (same id, incremented version) when no semantic field changes', async () => {
      ...
      const updated = await service.update(rule.id, { version: 3, notes: 'new', actorId: 'editor' } as any);
    ...
    it('rejects a second concurrent in-place edit that read the same stale version', async () => {
      ...
      await service.update(rule.id, { version: 1, notes: 'editor-a wins', actorId: 'editor-a' } as any);

      // editor-b read version 1 before editor-a's write landed.
      await expect(
        service.update(rule.id, { version: 1, notes: 'editor-b, stale', actorId: 'editor-b' } as any),
    ...
    it('creates a new versioned row when a semantic field changes, and disables the old row', async () => {
      ...
      const updated = await service.update(rule.id, { version: 1, keyword: 'new-keyword', actorId: 'editor' } as any);
    ...
    it('rejects with RuleVersionConflictException when version does not match', async () => {
      ...
      await expect(service.update(rule.id, { version: 2, notes: 'x', actorId: 'editor' } as any)).rejects.toBeInstanceOf(
        RuleVersionConflictException,
      );
    });

    it('rejects with RuleNotFoundException for an unknown id', async () => {
      await expect(service.update('missing-id', { version: 1, actorId: 'editor' } as any)).rejects.toBeInstanceOf(RuleNotFoundException);
    });

    it('does not conflict with its own prior version when re-enabling/versioning', async () => {
      const rule = makeRule({ version: 1, keyword: '肿瘤', level: 'RED', matchField: 'REPORT_TEXT', matchMode: 'CONTAINS' });

      // Editing category only (no semantic change) should not conflict with itself.
      await expect(service.update(rule.id, { version: 1, category: 'demo', actorId: 'editor' } as any)).resolves.toBeDefined();
    });

    it('rejects a semantic edit that would conflict with a different enabled rule', async () => {
      makeRule({ keyword: 'existing', level: 'RED', matchField: 'REPORT_TEXT', matchMode: 'CONTAINS', isEnabled: true });
      const rule = makeRule({ keyword: 'other', level: 'RED', matchField: 'REPORT_TEXT', matchMode: 'CONTAINS', isEnabled: true, version: 1 });

      await expect(service.update(rule.id, { version: 1, keyword: 'existing', actorId: 'editor' } as any)).rejects.toBeInstanceOf(
    ...
    it('supports disabling a rule in place (stop)', async () => {
      ...
      const updated = await service.update(rule.id, { version: 1, isEnabled: false, actorId: 'editor' } as any);
