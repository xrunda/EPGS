...
**科室范围收窄（2026-09-11 修订）**：本次不做科室级别的访问控制。只要是医生
（或任何被授予角色的账号），默认可见全院所有科室数据，不再提供按科室限制的
选择界面。原因：科室粒度的权限划分依赖一套权威的科室主数据，而这套数据应该
从医院 HIS 系统同步获得，不值得在当前阶段为此单独维护一份手工维护的科室清单。
待日后与 HIS 打通、有可信科室数据源时，再重新引入按科室限制的功能。`GET
/api/monitor/departments` 接口和科室多选 UI 均从本次范围移除；`app_user_access.
departmentScope` 字段在数据库层保留（避免破坏性迁移），但本次 API 与前端固定
按"全部科室"处理，不对外暴露该字段的编辑入口。
...
| 角色         | 用户列表/新增/启停/删除 | 分配角色（全院范围） | 重置密码 |
| ------------ | ------------------------ | -------------------- | -------- |
| `USER_ADMIN` | ✅                        | ✅                    | ✅       |
| 其他角色     | ❌ 403                    | ❌ 403                | ❌ 403   |
...
- **冷启动**：功能上线后，运维通过 CLI 给至少一个现有管理员账号追加
  ...
  ```
  （`assign-access` 是整表替换，需带上该账号原有的其它角色，避免覆盖丢失；不加
  `--departments` 即为全院范围，与本次范围收窄后的默认行为一致。）
...
- 展示字段：账号（`username`）、显示名（`displayName`）、启用状态
  （`isActive`）、已分配角色（`roles`，可能为空 = 无权限）、患者详情脱敏状态
  （`patientDetail`）、创建时间。不展示科室范围字段（本次范围收窄，详见上文
  "科室范围收窄"说明）。
...
- 密码由管理员在表单中手动设置两次（沿用 CLI 现状，不做邮件/短信下发），提交后
  ...
  方式保持一致。
- 提交后立即可在同一表单（或紧接着的引导步骤）分配角色，避免创建后忘记授权
  导致新账号登录即全部 403。
...
- 不提供科室范围选择——本次范围收窄，所有账号固定为全院可见（后端整表替换
  授权时 `departmentScope` 固定写空数组）。
...
| `PUT`    | `/api/users/:username/access`     | `USER_ADMIN` | 整表替换角色/患者详情（不接受 `departmentScope` 参数，服务端固定写空数组） |

响应、日志均不得包含密码或密码哈希，沿用 `docs/auth.md` 现有约束。`GET
/api/monitor/departments` 本次不实现（详见上文"科室范围收窄"说明）。
...
- 弹窗内为列表 + 新建/编辑表单的组合，参考 `RulesPanel` + `RulesModal` 的
  拆分方式。
- 角色多选为受控组件，无需引入组件库，手写 CSS 沿用 `tokens.css` 设计变量。
...
- 批量导入用户（CSV）——本次只做单个创建，参考规则模块的导入模式，后续如有
  需要再扩展。
- 科室级别的访问控制——`GET /api/monitor/departments` 接口、科室多选 UI、
  `departmentScope` 的编辑入口均不实现，待接入 HIS 系统获得权威科室数据源后
  再重新引入（详见文首"科室范围收窄"说明）。

## 待确认的实现细节（进入开发前）

1. `USER_ADMIN` 的 Prisma 迁移文件命名与 issue 编号。
2. `/api/users` 列表是否需要按角色筛选（当前草案只定了账号/显示名/启用状态
   筛选）。
3. 前端弹窗是"新建时强制紧接着填角色"，还是"新建后可先关闭、稍后再从列表
