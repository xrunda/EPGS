...
- [ ] **回滚基线确认**：日志中**没有**任何分类循环活动的痕迹；工作台、红黄绿、
      ...
      的主等级标签改为「红色关注 / 黄色关注 / 绿色关注」（issue #92 的文案统一，只有
      文字变长，没有新增控件）；此外列表**只多出**来源徽标（`NONE` 的行不显示徽标），
      详情抽屉**只多出**「AI 语义发现」区块——因为没有记录带 AI 等级，该区块此时应
      完全不出现。
...
- [ ] **医生端展示已复核（PR-B）**：打开开关前，用一条已判读的记录在工作台确认
      列表行出现来源徽标（关键词 / AI 语义 / 两者）、详情抽屉的「AI 语义发现」区块
      显示了名称、颜色、把握度、解释句与证据片段，且证据与报告正文逐字一致。
      **同时确认预警链接 H5 页面没有出现任何 AI 内容**——通知形态是被冻结的面，
      链接可以被转发到工作台之外。
...
- 立即止血：`SEMANTIC_REPORT_ENABLED=false` → `bash start.sh nopull`（§7.1）。
  注意**已产生的 AI 等级会保留**，不会自己降回去，因此工作台上的来源徽标与
  「AI 语义发现」区块也照旧显示——关掉开关只停止**新的**判读。
