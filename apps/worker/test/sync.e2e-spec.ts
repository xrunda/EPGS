...
describe('Sync job (e2e, real Postgres)', () => {
  ...
  beforeEach(async () => {
    ...
    // Clean slate between tests, respecting FK order (matches -> records;
  ...
  it('DB availability probe (informational, always runs)', () => {
    if (!dbAvailable) {
      // eslint-disable-next-line no-console
      console.warn(
        'sync.e2e-spec.ts: all subsequent cases are NO-OPS because no live Postgres was reachable.',
      );
    }
  ...
  itWithDb(
    ...
    async () => {
      ...
      await runSync(
        ...
        { ...baseOptions, lookbackMinutes: 60 * 24 * 30 },
        silentLogger,
      );
  ...
  itWithDb(
    'normal incremental sync: a second run only picks up newly-updated records after the first cursor',
    async () => {
      const items: PacsReportDto[] = [
        makeReport({
          reportId: 'INC-1',
          sourceRecordId: 'ACC-INC-1',
          sourceUpdatedAt: new Date('2026-08-21T01:00:00Z'),
        }),
      ];
      ...
      items.push(
        makeReport({
          reportId: 'INC-2',
          sourceRecordId: 'ACC-INC-2',
          sourceUpdatedAt: new Date('2026-08-21T01:10:00Z'),
        }),
      );
  ...
  itWithDb(
    'same sourceUpdatedAt timestamp on multiple records: all are processed without collision',
    async () => {
      const sameTs = new Date('2026-08-21T02:00:00Z');
      const items: PacsReportDto[] = [
        makeReport({ reportId: 'TS-A', sourceRecordId: 'ACC-TS-A', sourceUpdatedAt: sameTs }),
        makeReport({ reportId: 'TS-B', sourceRecordId: 'ACC-TS-B', sourceUpdatedAt: sameTs }),
        makeReport({ reportId: 'TS-C', sourceRecordId: 'ACC-TS-C', sourceUpdatedAt: sameTs }),
      ];
      const adapter = new StubAdapter(items);
      const summary = await runSync(
        prisma,
  ...
  itWithDb(
    'a report with an amended sourceUpdatedAt (content change, same natural key) re-triggers matching',
    async () => {
      await seedRedRule();
  ...
  itWithDb(
    'a late-arriving update within the look-back window is picked up even after the previous cursor advanced past it',
    async () => {
      const first = makeReport({
        reportId: 'LATE-1',
        sourceRecordId: 'ACC-LATE-1',
        sourceUpdatedAt: new Date('2026-08-21T04:00:00Z'),
      });
      ...
      const late = makeReport({
        reportId: 'LATE-2',
        sourceRecordId: 'ACC-LATE-2',
        sourceUpdatedAt: new Date('2026-08-21T03:59:00Z'),
      });
      ...
      const summary = await runSync(
        prisma,
        adapter,
        new Date('2026-08-21T04:10:00Z'),
        { ...baseOptions, lookbackMinutes: 10 },
        silentLogger,
      );
      expect(summary.status).toBe(SyncJobStatus.SUCCEEDED);
  ...
  itWithDb(
    'process interruption: a FAILED run does not advance the cursor past unprocessed data',
    async () => {
      const items = [
        makeReport({
          reportId: 'INT-1',
          sourceRecordId: 'ACC-INT-1',
          sourceUpdatedAt: new Date('2026-08-21T05:00:00Z'),
        }),
      ];
      ...
      const recoverySummary = await runSync(
        prisma,
        workingAdapter,
  ...
  itWithDb(
    'source unavailable (503-style transient error): retries then marks FAILED without losing the cursor position',
    async () => {
      let calls = 0;
  ...
  itWithDb('a single malformed/failing record does not block the rest of the batch', async () => {
    await seedRedRule();
    const good1 = makeReport({
      reportId: 'BAD-BATCH-1',
      sourceRecordId: 'ACC-BAD-1',
      sourceUpdatedAt: new Date('2026-08-21T07:00:00Z'),
    });
    const good2 = makeReport({
      reportId: 'BAD-BATCH-3',
      sourceRecordId: 'ACC-BAD-3',
      sourceUpdatedAt: new Date('2026-08-21T07:02:00Z'),
    });
    ...
    // error summary must reference only source identifiers, never patient name
    expect(jobLog?.errorSummary).not.toContain('测试患者');
  });

  itWithDb(
    'two workers running concurrently against the same batch do not create duplicate records',
    async () => {
      await seedRedRule();
      const items = [
        makeReport({
          reportId: 'CONC-1',
          sourceRecordId: 'ACC-CONC-1',
          sourceUpdatedAt: new Date('2026-08-21T08:00:00Z'),
          reportContent: '胃体见癌灶',
          diagnosis: '胃癌',
        }),
        makeReport({
          reportId: 'CONC-2',
          sourceRecordId: 'ACC-CONC-2',
          sourceUpdatedAt: new Date('2026-08-21T08:01:00Z'),
        }),
      ];
      ...
      // (reportContent: "胃体见癌灶") and IMPRESSION (diagnosis: "胃癌") -
      ...
      expect(new Set(matches.map((m) => m.matchedField))).toEqual(
...
class StubAdapter implements PacsRisAdapter {
  ...
  async fetchReports(params: FetchReportsParams): Promise<FetchReportsResult> {
    ...
    const filtered = this.items.filter(
      (i) => i.sourceUpdatedAt >= params.since && i.sourceUpdatedAt < until,
    );
